...
{
	typedef boost::function<void(void)> CookieFunc;

public:
	FixedBuffer()
		: m_pCur(m_Data)
	{
		SetCookie(boost::bind(&FixedBuffer<SIZE>::cookieStart));
	}

	~FixedBuffer() {
		SetCookie(boost::bind(&FixedBuffer<SIZE>::cookieEnd));
	}
	...
	void		SetCookie(CookieFunc cookieFunc) { m_CookieFunc = cookieFunc; }
